...
class ProductInfo(BaseModel):
    ...
    color: str = Field(default="Not specified", description="Procut color if mentioned")

root_agent = Agent(
    model='gemini-2.5-flash',
    name='product_extractor',
    description='Extracts product information from user messages & returns structured JSON',
    instruction="""You are a Product Information Extractor
    Your task:
    - Read the user's message about the product
    - Extract: product_name, price, storage & color (if mentioned)
    - Respond ONLY with valid JSON matching this format
    
    {
        "product_name": "product name is here",
        "price": 999.99,
        "storage": "256BG",
        "color": "Space Black"
    }
