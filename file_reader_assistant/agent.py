...
root_agent = LlmAgent(
    ...
    description='Helps users read & explore files using MCP tools',
    instruction="""
...
# FOR WINDOWS
# I wasn't able to spawn the mcp server as per the normal setup
# So I installed the mcp-server-filesystem using the command:
# npm install -g @modelcontextprotocol/server-filesystem
# Then verified if the python environment could access it using the command:
# where mcp-server-filesystem
# Finally called this command instead of calling npx

# Setup for linux / Normal setup as per docs
#McpToolset(
#            connection_params=StdioConnectionParams(
#                server_params=StdioServerParameters(
#                    command='npx',
