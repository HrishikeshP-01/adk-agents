...
Demonstrates strategic tool combination, error handling & agent-as-tool pattern
"""
...
ORDERS_DB = {
    ...
    "ORD456": {"status": "delivered","total": 149.99, "customer": "jane@email.com"}
}

# Tool 1: Check order status
def check_order_status(order_id: str)->dict:
    """
    Checks the currrent status of a customer's order
    ...
    if not order_id.startswith('ORD'):
        return {
            ...
            'error_message': 'Order IDs must start with ORD'
        }
...
# Tools 3: Escalate to supervisor
def escalate_to_supervisor(issue_summary: str, order_id: str)-> dict:
    ...
    """
    # Generate ticket ID
    ticket_id = f'TICKET{hash(issue_summary)%1000:04d}'
    return {
        'status': 'success',
        'ticket_id': ticket_id,
...
# Create customer support agents with strategic instructions
root_agent = LlmAgent(
    model='gemini-2.5-flash',
    name='customer_support_agent',
    description='Handles customer inquiries about orders & refunds with comprehensive error handling',
    instruction="""
        You are a helpful & empathetic customer support agent for an e-commerce company.

        # Your Capabilties
        You have 3 tools available:
        1. check_order_status(order_id) - Check the status of the order
        ...
        3. Handle the result:
            ...
            - If error_type='invalid_format': Explain that order IDs start with "ORD"
        ## For Refund Requests:
        1. Express empathy for their situation
        2. FIRST use check_order_status to verify the order exists
        3. If order not found, connot proceed with refund - ask customer to verify order ID
        4. If order exists, use process_refund with order_id & customer's reason
        5. Handle refund result:
            - If status='success': Confirm the refund with reference number, amount & timeframe
            - If error_type='order_not_found': This shouldn't happen if step 2 succeeded, but apologize & verify
            - If error_type='cannot_refund': Explain why (order status) & offer alternatives or escalation
        ...
        - Ask them to provde the order ID in correct format
        ...
        - Customer is fustrated or angry & requests supervisor
        - Issue can't be resolved with available tools
        ...
        # Communication style
        - Always be polite, professional & empathetic
